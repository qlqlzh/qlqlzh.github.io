...
public class maze_builder 
{
	...
	//							    	             W      E      N      S
	static int[][] moving = new int[][]{{% raw %} {0,-2}, {0,2}, {-2,0}, {2,0} {% endraw %}};		
	//	    							                 W       E       N     S
	static int[][] destruction = new int[][]{% raw %}{ {0,-1}, {0,1}, {-1,0}, {1,0} {% endraw %}};
	...
	{			
		...
		while(true)
		{
			...
			if(dir.size() > 1)
			{
				...
				//static int[][] destruction = new int[][]{{0,-1},{0,1},{-1,0},{1,0}};				
...
새로운 통로를 뚫을수 있는 곳이 있을대까지, forwarding함수에서 뚫었던 통로를 거슬로 올라간다.(bacwarding함수의 기능). 
			...
			{%>
			  ...
			  <%if(maze[i][j]==1)
				{%>						
					...
					$('#'+loc).attr('bgcolor', 'black');											
			  <%}
				else
				{%>
					<!--$('#'+loc).attr('style', 'border:1px solid white');	-->		
			  <%}
			}
		}
		%>	
        //현재 위치는 빨간색으로 나타낸다.
		$('#'+maze_size).attr('bgcolor', 'red');	
	}
	...
<script>	
```
...
relocation함수 실행을 통해  빨간색 사각형 이동을 한다.
...
만약 빨간색 사각형이 미로 오른쪽 하단의 출구에 도착했다면, ajax로 비동기 통신을 해서 ajax에 경과시간을 기록해준다.(숫저퍼즐 페이지에서 비동기 통신 참고)
